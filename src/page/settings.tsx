...
export default function SettingsPage() {
  ...
  const steps: TourProps["steps"] = [
    {
      ...
      description: (
        <div>
          <p>
            请前往{" "}
            <a href="https://oc.sjtu.edu.cn/profile/settings" target="_blank">
  ...
  const handleTestToken = async () => {
    ...
    try {
      ...
      messageApi.success(
        `👋你好，${me.name}。欢迎使用 SJTU Canvas Helper👏`,
        2
      );
    } catch (e) {
      ...
      <Space direction="vertical" style={{ width: "100%" }}>
        ...
        >
          ...
          <Space wrap>
            ...
            <Form.Item>
              <Button onClick={handleTestToken}>测试 Token</Button>
            </Form.Item>
            <Form.Item>
              <Button onClick={handleTestApiKey}>测试 API KEY</Button>
            </Form.Item>
            <Form.Item>
              {!rawConfig && <Button onClick={getRawConfig}>显示配置</Button>}
              {rawConfig && (
                <Button onClick={() => setRawConfig("")}>隐藏配置</Button>
              )}
            </Form.Item>
            <Form.Item>
              <Button onClick={() => setShowLogModal(true)}>查看日志</Button>
            </Form.Item>
            <Form.Item>
